...
float cRetargetBone::Length( float skeletonHeight ) const
{
    ...
    if( mChildren.Num() > 1 )
    {
        if( mID == OptiTrackBoneID::Hips )
        {
            cRetargetBone* childBone = FindChildByID( OptiTrackBoneID::Spine );
            return childBone ? childBone->mOffsetTransform.GetTranslation().Size() : 0;
        }
        else if( mID == OptiTrackBoneID::Spine || mID == OptiTrackBoneID::Spine1 )
        {
            if( cRetargetBone* childBone = FindChildByID( OptiTrackBoneID::Neck ) )
                return childBone->mOffsetTransform.GetTranslation().Size();
        }
        else if( mID == OptiTrackBoneID::Neck )
        {
            if( cRetargetBone* childBone = FindChildByID( OptiTrackBoneID::Head ) )
                return childBone->mOffsetTransform.GetTranslation().Size();
        }

        for( const cRetargetBone* childBone : mChildren )
        {
            // Look for first bone with valid ID
            if( childBone->mID != OptiTrackBoneID::UndefinedBone )
                return childBone->mOffsetTransform.GetTranslation().Size();
        }
    }
...
void cRetargetSkeleton::Create( const FLiveLinkSkeletonStaticData& skeletonData, const FLiveLinkAnimationFrameData& frameData, 
    const TMap<FString, OptiTrackBoneID>& boneNameMap )
{
	...
	for( int i = 0; i < skeletonData.GetBoneNames().Num(); i++ )
	{
        ...
        if( const OptiTrackBoneID* boneID = boneNameMap.Find( skeletonData.GetBoneNames()[i].ToString() ) )
            bone.mID = *boneID;

        if( bone.mID == OptiTrackBoneID::Origin || bone.mID == OptiTrackBoneID::Root )
...
void cRetargetSkeleton::Create( const FReferenceSkeleton& refSkeleton, const TMap<FString, OptiTrackBoneID>& boneNameMap )
{
	...
	for( int i = 0; i < boneInfos.Num(); i++ )
	{
        ...
        if( const OptiTrackBoneID* boneID = boneNameMap.Find( boneInfo.Name.ToString() ) )
            bone.mID = *boneID;
			...
			if( FTransform* trSrc = tPose.Find( bone.mID ) )
			{
				...
				else
				{
					...
					if( const cRetargetBone* srcBone = sourceSkeleton.FindBoneByID( bone.mID ) )
					{
                        float scale = 1.0f;
                        {
                            float boneLength = bone.Length( Height() );
                            if( boneLength > 0 )
                                scale = srcBone->Length( sourceSkeleton.Height() ) / boneLength;
                        }
                        scale /= heightScale;
...
void cRetargetSkeleton::SetHipOffsetByLegLength()
{
    ...
    hip->UpdateTransform( true );

    float hipHeight = 0;
    const cRetargetBone* bone = footBone;
    while( bone && bone->mParent && bone->mID != OptiTrackBoneID::Hips )
    {
        switch( bone->mID )
        {
        case OptiTrackBoneID::LeftUpLeg:
        case OptiTrackBoneID::LeftToeBase:
            // Vertical distance
            hipHeight += ( bone->mParent->mFinalTransform.GetTranslation() - bone->mFinalTransform.GetTranslation() ).Z;
            break;
        default:
            // For other bones, actual joint to joint distance
            hipHeight += ( bone->mParent->mFinalTransform.GetTranslation() - bone->mFinalTransform.GetTranslation() ).Length();
            break;
        }

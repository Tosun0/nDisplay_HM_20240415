...
5.3 v3 (2024-04-15)
----
- Added Male and Female Motive Avatars to the Plugins Content folder for retargeting and data validation
- Added CinePuck Static Mesh to the Plugins Content folder
- Added Pose for m_med_nrw MetaHumans to use for easier setup in retargeting, including with finger data
- Included the ability to select a Rigid Body asset to be a streamed in as a Camera Role
- Clicking off and on the Live Link Display now updates the assets in the details panel when switching takes in Motive
- Fixed an issue where Skeletons would be vertically offset from their position if the Live Link Connection was made when the Actors feet were not on the ground plane
- Updated the “OptiTrackLiveLinkRetarget” asset for Animation Blueprints to read in any Bone Naming conventions and at proper rotation offsets.
- Fixed an issue where the Live Link Display was not controlling Trained Markersets data properly 
- Fixed a variety of Issues with Meshes attached to the Live Link Display not undoing properly when translating the Display
- Fixed an issue when connecting the Motive stream to Unreal it would force Motive to switch to FBX bone naming conventions
- Removed unnecessary information in the Live Link Window UI
- Fixed some Display Name issues in the Live Link Window UI
